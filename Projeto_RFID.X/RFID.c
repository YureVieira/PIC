...
inline void RFID_init() {
	...
	SPI_TRIS&=~(1<<RFID_RESET);
	SPI_PORT&=~(1<<RFID_RESET) ;
...
void PCD_WriteRegister(	unsigned char reg,		///< The register to write to. One of the PCD_Register enums.
	...
	SPI_transfer(value);
	SPI_PORT|=(1<<RFID_CS) ;		// Release slave again
...
unsigned char PCD_CalculateCRC(	unsigned char *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
	...
	PCD_WriteRegisterVector(FIFODataReg, length, data);	// Write data to the FIFO
 ...
 */
void PCD_Init() {
	if (SPI_PORT & (1<<RFID_RESET) == 0) {	//The MFRC522 chip is in power down mode.
		SPI_PORT|=(1<<RFID_RESET) ;		// Exit power down mode. This triggers a hard reset.
		// Section 8.8.2 in the datasheet says the oscillator start-up time is the start up time of the crystal + 37,74�s. Let us be generous: 50ms.
		__delay_ms(50);
	}
...
//    PCD_WriteRegisterVector(FIFODataReg, 25, ZEROES); // write 25 unsigned chars of 00h to FIFO
...
unsigned char PCD_CommunicateWithPICC(	unsigned char command,		///< The command to execute. One of the PCD_Command enums.
	...
	PCD_SetRegisterBitMask(FIFOLevelReg, 0x80);			// FlushBuffer = 1, FIFO initialization
	PCD_WriteRegisterVector(FIFODataReg, sendLen, sendData);	// Write sendData to the FIFO
	...
	if (backData && backLen) {
		...
		*backLen = n;											// Number of unsigned chars returned
		PCD_ReadRegisterVector(FIFODataReg, n, backData, rxAlign);	// Get received data from FIFO
...
unsigned char PICC_HaltA() {
	...
	result = PCD_TransceiveData(buffer, sizeof(buffer), NULL, NULL, NULL,0,0);
...
const char *PICC_GetTypeName(unsigned char piccType	///< One of the PICC_Type enums.
	...
	switch (piccType) {
		...
		case PICC_TYPE_MIFARE_MINI:		return "MIFARE Mini, 320 unsigned chars";break;
...
void PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
    ...
    unsigned char invertedError;		// 1 if one of the inverted nibbles did not match
    unsigned char g[4];				// Access bits for each of the four groups.
    unsigned char group;				// 0-3 - active group for access bits
    unsigned char firstInGroup;		// 1 for the first block dumped in the group
